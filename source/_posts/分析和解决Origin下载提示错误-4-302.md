...
对于这些国际的游戏客户端，出现下载问题第一反应应该就是被RST了。所以我第一时间沿袭了魔法的姿势完成了游戏下载。
但是这样会消耗很多的魔法力（流量），毕竟一个客户端就几十G，而且听说其他人下载还是有正常的，所以不太像是直接被RST了。
...
> update: 这个102.52.51.0/24段的IP是联通的缓存劫持。[来源](https://www.v2ex.com/t/537381)

~~那么看起来这应该是EA对国内CDN的一个部署，没有用DNS解析到国内节点IP的方式，而是直接重定向到国内的节点IP域名，~~这是被联通做了HTTP缓存劫持了，Origin客户端并不支持重定向。
随后我又使用魔法姿势然后抓包，正常下载确实没有重定向，而是直接下载文件，所以客户端就没有报错了。

# 3 换一个CDN？

根据一个女装群大佬们的说法，修改`EACore.ini`这个文件使用**Akamai**的CDN下载。
然而结果还是直接报错，抓包看跟EA自己的CDN走向一样。

![Akamai CDN 302](./akamai-302.png)

Akamai的CDN也是用302直接重定向到~~国内的节点~~联通缓存劫持IP。从`http://origin-a.akamaihd.net/***`定向到了`http://59.80.44.100/origin-a.akamaihd.net/***`。
